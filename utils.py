...
def load_weights(G1, G2, D, state_dict, weights_root, experiment_name,
                 name_suffix=None, G1_ema=None, G2_ema=None, strict=True, load_optim=True):
    root = '/'.join([weights_root, experiment_name])
    if name_suffix:
    ...
    if G1 is not None:
        G1.load_state_dict(
            torch.load('%s/%s.pth' % (root, join_strings('_', ['G1', name_suffix]))),
            strict=strict)
        if load_optim:
            G1.optim.load_state_dict(
                torch.load('%s/%s.pth' % (root, join_strings('_', ['G1_optim', name_suffix]))))
    if G2 is not None:
        G2.load_state_dict(
            torch.load('%s/%s.pth' % (root, join_strings('_', ['G2', name_suffix]))),
            strict=strict)
        if load_optim:
            G2.optim.load_state_dict(
                torch.load('%s/%s.pth' % (root, join_strings('_', ['G2_optim', name_suffix]))))
    if D is not None:
        D.load_state_dict(
    ...
    if G1_ema is not None:
        G1_ema.load_state_dict(
            torch.load('%s/%s.pth' % (root, join_strings('_', ['G1_ema', name_suffix]))),
            strict=strict)
    if G2_ema is not None:
        G2_ema.load_state_dict(
            torch.load('%s/%s.pth' % (root, join_strings('_', ['G2_ema', name_suffix]))),
            strict=strict)
...
def sample(G1, G2, z_, y_, config):
    with torch.no_grad():
        z_.sample_()
        y_.sample_()
        if config['parallel']:
            G_h = nn.parallel.data_parallel(G1, z_)
            G_z = nn.parallel.data_parallel(G2, G_h)
        else:
            G_h = G1(z_)
            G_z = G2(G_h)
        return G_z, y_
...
def sample_sheet(G1, G2, classes_per_sheet, num_classes, samples_per_class, parallel,
    ...
    for i in range(num_classes // classes_per_sheet):
        ...
        for j in range(samples_per_class):
            ...
            with torch.no_grad():
                if parallel:
                    h = nn.parallel.data_parallel(G1, z_[:classes_per_sheet])
                    o = nn.parallel.data_parallel(G2, h)
                else:
                    h = G1(z_[:classes_per_sheet])
                    o = G2(h)
...
def interp_sheet(G1, G2, num_per_sheet, num_midpoints, num_classes, parallel,
                 samples_root, experiment_name, folder_number, sheet_number=0,
                 fix_z=False, fix_y=False, device='cuda'):
    # Prepare zs and ys
    if fix_z:  # If fix Z, only sample 1 z per row
        zs = torch.randn(num_per_sheet, 1, G1.dim_z, device=device)
        zs = zs.repeat(1, num_midpoints + 2, 1).view(-1, G1.dim_z)
    else:
        zs = interp(torch.randn(num_per_sheet, 1, G1.dim_z, device=device),
                    torch.randn(num_per_sheet, 1, G1.dim_z, device=device),
                    num_midpoints).view(-1, G1.dim_z)
    if fix_y:  # If fix y, only sample 1 z per row
        ys = sample_1hot(num_per_sheet, num_classes)
        ys = G1.shared(ys).view(num_per_sheet, 1, -1)
        ys = ys.repeat(1, num_midpoints + 2, 1).view(num_per_sheet * (num_midpoints + 2), -1)
    else:
        ys = interp(G1.shared(sample_1hot(num_per_sheet, num_classes)).view(num_per_sheet, 1, -1),
                    G1.shared(sample_1hot(num_per_sheet, num_classes)).view(num_per_sheet, 1, -1),
    ...
    with torch.no_grad():
        if parallel:
            out_hs = nn.parallel.data_parallel(G1, zs).data.cpu()
            out_ims = nn.parallel.data_parallel(G2, out_hs).data.cpu()
        else:
            out_hs = G1(zs).data.cpu()
            out_ims = G1(out_hs).data.cpu()
...
def accumulate_standing_stats(G1, G2, z, y, nclasses, num_accumulations=16):
    initiate_standing_stats(G1)
    initiate_standing_stats(G2)
    G1.train()
    G2.train()
    for i in range(num_accumulations):
        with torch.no_grad():
            ...
            h = G1(z)  # No need to parallelize here unless using syncbn
            x = G2(h)  # No need to parallelize here unless using syncbn
    # Set to eval mode
    G1.eval()
    G2.eval()
